...
static void binder_free_ref(struct binder_ref *ref)
{
	...
	if (ref->freeze)
		kmem_cache_free(binder_ref_freeze_pool, ref->death);
	if (ref->death)
...
static void binder_deferred_fd_close(int fd)
{
	...
	twcb = kmem_cache_zalloc(binder_twcb_pool, GFP_KERNEL);
	if (!twcb)
		return;
			       ...
			       struct binder_transaction *in_reply_to)
{
	 ...
	 * target thread.
	 */
	fixup = kmem_cache_zalloc(binder_fixup_pool, GFP_KERNEL);
	if (!fixup) {
		ret = -ENOMEM;
		goto err_alloc;
	}
